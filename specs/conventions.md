...
- Avoid combining `if` with variable assignment.  e.g. `if err := func(); err != nil { ... }`. Prefer separate lines per
  statement.
- Any code not intended to be publicly exported for use outside of this module should reside under an `internal`
  package.
...
- If necessary to declare a `.PHONY` target, declare it on the line preceding the target.  It is unfavorable to declare
  a single `.PHONY` listing all targets.
